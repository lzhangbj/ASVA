...
def load_video(video_path):
	...
	for i, frame in enumerate(itertools.takewhile(
		...
		if frame["pts"] + 1e-6 >= frame_timestamp: # avoid precision error
	...
	assert len(video_frames) == video_num_frame, f"video not long enough({len(video_frames)}) to sample 12 frames in 2 seconds"
...
if __name__ == "__main__":
	...
	parser.add_argument("--ref_audio", type=str, default="", help="path of audio to be reference")
	parser.add_argument("--ref_video", type=str, default="", help="path of video to be reference")
	...
	if args.ref_audio:
		ref_audio_waveform, ref_audio_sr = load_audio(args.ref_audio)
	else:
		ref_audio_waveform, ref_audio_sr = None, None
	
	if args.ref_video:
		ref_video = load_video(args.ref_video)
	else:
		ref_video = None
